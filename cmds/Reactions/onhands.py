...
def init():
    return [["onhands", onhands, "all", "all", "rlist"]]

async def onhands(client, message, command, messageArray, lang_u):
    ...
    if data["money"] < config.reaction_one:
        ...
        if len(message.mentions) == 0:
            if lang_u == "ru":
                e = discord.Embed(title="", description=f"<@{str(message.author.id)}>, укажи ник кого ты хочешь **взять на ручки**.", color=discord.Color(0x2F3136))
                e.timestamp = datetime.datetime.utcnow()
                e.set_footer(text=f"{message.author.display_name}", icon_url=message.author.avatar_url)
                return await message.channel.send(embed=e)
            elif lang_u == "en":
        ...
        if user_m["partner"] != "":
            if user_m["partner"] == str(message.author.id):
                if user_m_s["command_onhands_marry"] == 0:
                    photo = config.onhands
                    e = discord.Embed(title="Реакция: взять на ручки", description=f"<@{str(message.author.id)}> взял на ручки <@{str(message.mentions[0].id)}>", color=discord.Color(0x2F3136))
                    ...
                    susers.update_one({"disid": str(message.author.id), "guild": str(message.guild.id)}, { "$set": { "money": author_m["money"] - config.reaction_one, "moneystats": ms } })
                    return await message.channel.send(embed=e)
            else:
        ...
        if lang_u == "ru":
            e = discord.Embed(title="Реакция: взять за руку", description=f"<@{str(message.mentions[0].id)}>, <@{message.author.id}> хочет взять тебя **на ручки**. Что ответишь?", color=discord.Color(0x2F3136))
            e.timestamp = datetime.datetime.utcnow()
            e.set_footer(text=f"{message.author.display_name}", icon_url=message.author.avatar_url)
        ...
        if ms["history"]["reactions"]["view"] == 0:
            ms["history"]["reactions"]["view"] = 1
        ms["history"]["reactions"]["count"] -= config.reaction_one
        db.prof_ec_users.update_one({"disid": str(message.author.id), "guild": str(message.guild.id)}, { "$set": { "money": author_m["money"] - config.reaction_one, "moneystats": ms } })
        coll = db.reactions
        coll.insert_one({"message_id": str(message_s.id), "id": str(message.author.id), "mention_id": str(message.mentions[0].id), "time": int(time.time()) + 30, "react": 7, "guild_id": str(message.guild.id), "channel_id": str(message.channel.id), "lang": str(lang_u), "type": "reaction"})