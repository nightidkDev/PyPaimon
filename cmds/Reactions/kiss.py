...
def init():
    return [["kiss", kiss, "all", "all", "rlist"]]

async def kiss(client, message, command, messageArray, lang_u):
    ...
    if data["money"] < config.reaction_two:
        ...
        if len(message.mentions) == 0:
            if lang_u == "ru":
                e = discord.Embed(title="", description=f"<@{str(message.author.id)}>, укажи ник кого ты хочешь **поцеловать**.", color=discord.Color(0x2F3136))
                e.timestamp = datetime.datetime.utcnow()
                e.set_footer(text=f"{message.author.display_name}", icon_url=message.author.avatar_url)
                return await message.channel.send(embed=e)
            elif lang_u == "en":
                e = discord.Embed(title="", description=f"<@{str(message.author.id)}>, enter nick who do you want to **kiss**.", color=discord.Color(0x2F3136))
                e.timestamp = datetime.datetime.utcnow()
                e.set_footer(text=f"{message.author.display_name}", icon_url=message.author.avatar_url)
                return await message.channel.send(embed=e)
        ...
        if user_m["partner"] != "":
            if user_m["partner"] == str(message.author.id):
                if user_m_s["command_kiss_marry"] == 0:
                    photo = config.kiss
                    e = discord.Embed(title="Реакция: поцелуй", description=f"<@{str(message.author.id)}> поцеловал(-а) <@{str(message.mentions[0].id)}>", color=discord.Color(0x2F3136))
        ...
        if lang_u == "ru":
            e = discord.Embed(title="Реакция: поцелуй", description=f"<@{str(message.mentions[0].id)}>, <@{message.author.id}> тебя хочет **поцеловать**. Что ответишь?", color=discord.Color(0x2F3136))
            e.timestamp = datetime.datetime.utcnow()
            e.set_footer(text=f"{message.author.display_name}", icon_url=message.author.avatar_url)
            ...
            e = discord.Embed(title="Reaction: kiss", description=f"<@{str(message.mentions[0].id)}>, <@{message.author.id}> wants to **kiss** you. What do you say?", color=discord.Color(0x2F3136))
            e.timestamp = datetime.datetime.utcnow()
            e.set_footer(text=f"{message.author.display_name}", icon_url=message.author.avatar_url)
            message_s = await message.channel.send(embed=e)
            await message_s.add_reaction("✅")
        ...
        coll = db.reactions
        coll.insert_one({"message_id": str(message_s.id), "id": str(message.author.id), "mention_id": str(message.mentions[0].id), "time": int(time.time()) + 30, "react": 1, "guild_id": str(message.guild.id), "channel_id": str(message.channel.id), "lang": str(lang_u), "type": "reaction"})