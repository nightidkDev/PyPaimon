...
def init():
    return [["virt", virt, "all", "all", "rlist"]]

async def virt(client, message, command, messageArray, lang_u):
    ...
    if data["money"] < config.reaction_two:
        ...
        if len(message.mentions) == 0:
            if lang_u == "ru":
                e = discord.Embed(title="", description=f"<@{str(message.author.id)}>, укажи ник с кем ты хочешь **повиртить**.", color=discord.Color(0x2F3136))
                e.timestamp = datetime.datetime.utcnow()
                e.set_footer(text=f"{message.author.display_name}", icon_url=message.author.avatar_url)
                return await message.channel.send(embed=e)
            elif lang_u == "en":
                e = discord.Embed(title="", description=f"<@{str(message.author.id)}>, enter nick who do you want to **virt**.", color=discord.Color(0x2F3136))
                e.timestamp = datetime.datetime.utcnow()
                e.set_footer(text=f"{message.author.display_name}", icon_url=message.author.avatar_url)
                return await message.channel.send(embed=e)
        ...
        if user_m["partner"] != "":
            if user_m["partner"] == str(message.author.id):
                if user_m_s["command_virt_marry"] == 0:
                    photo = config.virt
                    e = discord.Embed(title="Реакция: вирт", description=f"<@{str(message.author.id)}> и <@{str(message.mentions[0].id)}> **виртят**!", color=discord.Color(0x2F3136))
        ...
        if lang_u == "ru":
            e = discord.Embed(title="Реакция: вирт", description=f"<@{str(message.mentions[0].id)}>, с тобой хочет **повиртить** <@{message.author.id}>. Что ответишь?", color=discord.Color(0x2F3136))
            e.timestamp = datetime.datetime.utcnow()
            e.set_footer(text=f"{message.author.display_name}", icon_url=message.author.avatar_url)
            ...
            e = discord.Embed(title="Reaction: virt", description=f"<@{str(message.mentions[0].id)}>, <@{message.author.id}> wants to **virt** with you. What do you say??", color=discord.Color(0x2F3136))
            e.timestamp = datetime.datetime.utcnow()
            e.set_footer(text=f"{message.author.display_name}", icon_url=message.author.avatar_url)
            message_s = await message.channel.send(embed=e)
            await message_s.add_reaction("✅")
        ...
        coll = db.reactions
        coll.insert_one({"message_id": str(message_s.id), "id": str(message.author.id), "mention_id": str(message.mentions[0].id), "time": int(time.time()) + 30, "react": 3, "guild_id": str(message.guild.id), "channel_id": str(message.channel.id), "lang": str(lang_u), "type": "reaction"})